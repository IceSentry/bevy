...
pub struct OitResolveViewBindGroup(pub BindGroup);
...
pub fn prepare_oit_resolve_bind_group(
    ...
    if let Some(binding) = view_uniforms.uniforms.binding() {
        ...
        commands.insert_resource(OitResolveViewBindGroup(bind_group));
    }
    if let (Some(layers_binding), Some(layer_ids_binding)) =
        (buffers.layers.binding(), buffers.layer_ids.binding())
    {
        let bg = render_device.create_bind_group(
            "oit_resolve_layers_bind_group",
